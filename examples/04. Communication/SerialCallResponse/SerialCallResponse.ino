...
/*      Inovatic-ICT d.o.o									                                                                */
/*												                                                                                  */
/*      EMoRo Example:		  Serial Call and Response                                                        */
/*      Board name: 		    EMoRo 2560 v3.2			                                          			            */
/*      MCU name:		        ATMEGA2560-16					                                                          */
/*      Clock:			        16 MHz						                                                              */
/*      File description:                                                                                   */
/*            This program sends an ASCII A (byte of value 65) on startup and repeats that until it gets    */
/*            some data in. Then it waits for a byte in the serial port, and sends three sensor values      */
/*            whenever it gets a byte in.                                                                   */
/*      The circuit:                                                                                        */
/*         * potentiometers attached to analog inputs ADC_0 and ADC_1                                       */
/*            Attach the center pin of a 10k potentiometer to pin 3 (the right most pin) of ADC   connector */
/*            and the outside pins of a potentiometer to pin 1 (GND, the left most pin - marked with a      */
/*            small triangle) and pin 2 (+5V, the center pin) of ADC connector.                             */
/*         * pushbutton attached to digital IO_8, one connection to pin 1, one connection to pin 3          */
/*         * Serial connection to Processing, Max/MSP, or another serial application or Serial Monitor      */
/*      Necessary equipment:                                                                                */
...
/*                                                pin header male 3x1 RM 2.54mm)     2 pieces               */
...
void setup()
{
  ...
  pinMode(IO_8, INPUT_PULLUP);    // digital sensor is on digital pin IO_8 (Enable pullup);
  establishContact();             // call function "establishContact" - send a byte to establish contact until receiver responds 
}
...
void loop()
{
  if (Serial.available() > 0) {                             // if we get a valid byte, read analog ins
    ...
    firstSensor = analogRead(ADC_0)/4;                      // read first analog input, divide by 4 to make the range 0-255:
    delay(10);                                              // delay 10ms to let the ADC recover
    secondSensor = analogRead(ADC_1)/4;                     // read second analog input, divide by 4 to make the range 0-255
    thirdSensor = map(digitalRead(IO_8), 0, 1, 0, 255);     // read  switch, map it to 0 or 255
    
    // send sensor values:
    Serial.write(firstSensor);                              // write sensor value over serial communication
    Serial.write(secondSensor);                             // write sensor value over serial communication
    Serial.write(thirdSensor);                              // write sensor value over serial communication

    // if you want to use Serial monitor to check sensors and communicate comment the block above and uncomment the block below
    // send sensor values:
    //Serial.print("First sensor: ");                           // write header string
    //Serial.println(firstSensor);                              // write sensor value over serial communication
    //Serial.print("Second sensor: ");                          // write header string
    //Serial.println(secondSensor);                             // write sensor value over serial communication
    //Serial.print("Third sensor: ");                           // write header string
    //Serial.println(thirdSensor);                              // write sensor value over serial communication
  }
}

void establishContact() {
  while (Serial.available() <= 0) {                         // while there is no recieved data
    Serial.print('A');                                      // send a capital A
    delay(300);                                             // send data every 300 ms
  }
}
