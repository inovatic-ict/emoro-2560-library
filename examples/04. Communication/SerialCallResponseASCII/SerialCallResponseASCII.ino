...
/*      Inovatic-ICT d.o.o		                                                    							            */
/*												                                                                                  */
/*      EMoRo Example:		  Serial Call and Response in ASCII                                               */
/*      Board name: 		    EMoRo 2560 v3.2	                                          					            */
/*      MCU name:		        ATMEGA2560-16					                                                          */
/*      Clock:			        16 MHz						                                                              */
/*      File description:                                                                                   */
/*            This program sends an ASCII A (byte of value 65) on startup and repeats that until it gets    */
/*            some data in. Then it waits for a byte in the serial port, and sends three ASCII-encoded,     */
/*            comma-separated sensor values, truncated by a linefeed and carriage return, whenever it gets  */
/*            a byte in.                                                                                    */
/*      The circuit:                                                                                        */
...
void setup()
{
  ...
  pinMode(IO_8, INPUT_PULLUP);      // digital sensor is on digital pin IO_8
...
void loop()
{
  ...
  if (Serial.available() > 0) {                           // if we get a valid byte, read analog ins
    ...
    firstSensor = analogRead(ADC_0);                      // read first analog input
    secondSensor = analogRead(ADC_1);                     // read second analog input
    thirdSensor = map(digitalRead(IO_8), 0, 1, 0, 255);   // read  switch, map it to 0 or 255 
    
    // send sensor values:
    Serial.print(firstSensor);                            // send sensor value
    Serial.print(",");                                    // send comma delimiter
    Serial.print(secondSensor);                           // send sensor value
    Serial.print(",");                                    // send comma delimiter
    Serial.println(thirdSensor);                          // send sensor value
  }
}

void establishContact() {
  while (Serial.available() <= 0) {                       // while there is no recieved data
    Serial.println("Waiting for data");                    // send an initial string
