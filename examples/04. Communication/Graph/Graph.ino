...
/*      Clock:			        16 MHz						                                                              */
/*      File description:                                                                                   */
/*            A simple example of communication from the Arduino board to the computer: the value of        */
/*            analog input ADC_0 is sent out the serial port.  We call this "serial" communication because  */
/*            the connection appears to both the Arduino and the computer as a serial port, even though it  */
/*            may actually use a USB cable. Bytes are sent one after another (serially) from the Arduino to */
/*            the computer.                                                                                 */
/*            You can use the Arduino serial monitor to view the sent data, or it can be read by            */
/*            Processing, PD, Max/MSP, or any other program capable of reading data from a serial port.     */
/*            The Processing code below graphs the data received so you can see the value of the analog     */
...
void loop() {
  Serial.println(analogRead(ADC_0));          // send the value of analog input ADC_0
  delay(2);                                   // wait a bit for the ADC to stabilize after the last reading
}
