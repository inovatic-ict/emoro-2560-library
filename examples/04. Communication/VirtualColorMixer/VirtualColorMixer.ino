...
/*      Inovatic-ICT d.o.o				                                                    					            */
/*												                                                                                  */
/*      EMoRo Example:		  Virtual color mixer                                                             */
/*      Board name: 		    EMoRo 2560 v3.2                   			                                        */
/*      MCU name:		        ATMEGA2560-16					                                                          */
/*      Clock:			        16 MHz						                                                              */
/*      File description:                                                                                   */
/*            This example reads three analog sensors (potentiometers are easiest) and sends their values   */
/*            serially. The Processing and Max/MSP programs at the bottom take those three values and use   */
/*            them to change the background color of the screen.                                            */
/*      The circuit:                                                                                        */
/*           potentiometers attached to analog inputs ADC_0, ADC_1, and ADC_2.                              */
...
/*                          Analog signal source (e.g. 10k potentiometer, wires,                            */
...
const int redPin = ADC_0;		              // sensor to control red color
const int greenPin = ADC_1;	              // sensor to control green color
const int bluePin = ADC_2;		            // sensor to control blue color
...
void loop(){
  // send all analog values over serial communication
  Serial.print(analogRead(redPin));       // send value from redPin via serial communication
  Serial.print(",");                      // send delimiter via serial communication
  Serial.print(analogRead(greenPin));     // send value from greenPin via serial communication
  Serial.print(",");                      // send delimiter via serial communication
  Serial.println(analogRead(bluePin));    // send value from bluePin via serial communication
}
